...
function generateMongoDBURL(options) {
  ...
  if (username && options.password) {
    return 'mongodb://' + username + ':' + options.password + '@' +
      options.hostname + ':' + options.port + '/' + options.database;
  } else {
    return 'mongodb://' + options.hostname + ':' + options.port + '/' +
      options.database;
  }
}
...
exports.initialize = function initializeDataSource(dataSource, callback) {
  ...
  var url = s.url || generateMongoDBURL(s);
  s.db = s.db || {};
  s.db.address = url;
  s.db.collection = s.collection || 'AgendaJob';
